...
const rules = ref({
  ...
  phone: [{ match: /^1[3|4|5|6|7|8|9][0-9]\d{8}$/, message: '请输入正确的手机号' }],
  password: [
...
const handleSubmit = async ({ values, errors }: any) => {
  if (errors) {
    window.Message.error('请正确填写后修改密码')
    return
  }
...
<template>
  <a-modal :visible="show" draggable :title="title" :footer="false" @cancel="handleCancel">
    <a-form ref="formRef" :model="form" :rules="rules" @submit="handleSubmit">
      ...
      <a-button html-type="submit" type="primary" class="mgt-20" long>修改密码</a-button>
    </a-form>
  </a-modal>
</template>
